...
struct Skiplist<Key, Comparator>::SkiplistLevel {
  explicit SkiplistLevel(SkiplistNode* next, size_t span) : next(next), span(0){};
  void reset();
  ~SkiplistLevel() { reset(); }
  SkiplistNode* next;
  size_t span;
...
struct Skiplist<Key, Comparator>::SkiplistNode {
 public:
  ...
  const SkiplistNode* getNext(size_t level) const { return levels[level]->next; };
  ...
  void initLevel(size_t level) { levels[level] = new SkiplistLevel(nullptr, 0); };
  void reset();
  ~SkiplistNode();
  Key key;
...
template <typename Key, typename Comparator>
std::vector<Key> Skiplist<Key, Comparator>::getElementsByRange(int idx, size_t len) {
  if (idx < 0) {
    idx += _size;
  }
  if (idx < 0) return {};
...
template <typename Key, typename Comparator>
std::vector<Key> Skiplist<Key, Comparator>::getElementsByRevRange(int idx, size_t len) {
  if (idx < 0) {
    idx += _size;
  }
  if (idx < 0) return {};
...
Skiplist<Key, Comparator>::getFirstElementGt(const Key& key, bool eq) {
  ...
  for (int i = level - 1; i >= 0; --i) {
    while (node->getNext(i) &&
           (eq ? lt(node->getNext(i)->key, key) : lte(node->getNext(i)->key, key))) {
      node = node->getNext(i);
    }
  }
...
const typename Skiplist<Key, Comparator>::SkiplistNode* Skiplist<Key, Comparator>::getLastElementLt(
  ...
  for (int i = level - 1; i >= 0; --i) {
    while (node->getNext(i) &&
           (eq ? lte(node->getNext(i)->key, key) : lt(node->getNext(i)->key, key))) {
      node = node->getNext(i);
    }
  }
  return node;
...
template <typename Key, typename Comparator>
std::vector<Key> Skiplist<Key, Comparator>::getElements(size_t rank, size_t len) {
...
template <typename Key, typename Comparator>
std::vector<Key> Skiplist<Key, Comparator>::getElementsRev(size_t rank, size_t len) {
  if (rank >= _size || len == 0) return {};
